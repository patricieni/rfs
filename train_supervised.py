...
def parse_option():
    ...
    parser.add_argument('--use_trainval',
                        action='store_true',
                        help='use trainval set')
    ...
    # meta setting
    parser.add_argument('--n_test_runs',
                        type=int,
                        default=600,
                        metavar='N',
    ...
    opt.model_name = '{}_{}_lr_{}_decay_{}_trans_{}'.format(
        opt.model, opt.dataset, opt.learning_rate, opt.weight_decay,
        opt.transform)
    ...
    print("Device count: {0}".format(opt.n_gpu))
...
def main():
    ...
    if opt.dataset == 'miniImageNet':
        ...
        # Interesting one here - it's either using 80 to
        if opt.use_trainval:
            n_cls = 80
        else:
            n_cls = 64
